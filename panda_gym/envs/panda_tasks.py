...
from panda_gym.envs.core import RobotTaskEnv, RobotTaskDoubleEnv
...
from panda_gym.envs.tasks.reach_double import ReachDouble
...
class PandaReachEnv(RobotTaskEnv):
    ...
    Args:
        ...
        control_type (str, optional): "ee" to control end-effector position or "joints" to control joint values.
            Defaults to "ee".
        goal_random (bool, optional): Choice of generating desired goal position to be random or fixed. Default to "True".
        renderer (str, optional): Renderer, either "Tiny" or OpenGL". Defaults to "Tiny" if render mode is "human"
            and "OpenGL" if render mode is "rgb_array". Only "OpenGL" is available for human render mode.
        render_width (int, optional): Image width. Defaults to 720.
        render_height (int, optional): Image height. Defaults to 480.
    ...
    def __init__(
        ...
        sim = PyBullet(render_mode=render_mode, renderer=renderer)
        robot = Panda(sim, block_gripper=True, base_position=np.array([-0.6, 0.0, 0.0]), control_type=control_type)
        task = Reach(sim, reward_type=reward_type, get_ee_position=robot.get_ee_position, goal_random=goal_random)
        super().__init__(
            robot,
            task,
            render_width=render_width,
...
class PandaReachDoubleEnv(RobotTaskDoubleEnv):
    """Reach one of 2 goals task with Panda robot.
    ...
    Args:
        ...
        control_type (str, optional): "ee" to control end-effector position or "joints" to control joint values.
            Defaults to "ee".
        goal_random (bool, optional): Choice of generating desired goal positions to be random or fixed. Default to "True".
        renderer (str, optional): Renderer, either "Tiny" or OpenGL". Defaults to "Tiny" if render mode is "human"
            and "OpenGL" if render mode is "rgb_array". Only "OpenGL" is available for human render mode.
        render_width (int, optional): Image width. Defaults to 720.
        render_height (int, optional): Image height. Defaults to 480.
    ...
    def __init__(
        ...
        sim = PyBullet(render_mode=render_mode, renderer=renderer)
        robot = Panda(sim, block_gripper=True, base_position=np.array([-0.6, 0.0, 0.0]), control_type=control_type)
        task = ReachDouble(sim, reward_type=reward_type, get_ee_position=robot.get_ee_position, goal_random=goal_random)
        super().__init__(
            robot,
            task,
            render_width=render_width,
